...
export default function RRegistration() 
{
  ...
  const handleSubmit = async (e) => 
  {
    ...
    try 
    {
      const response = await axios.post('http://localhost:2032/insertrecruiter', formData);
  ...
  return (
    <div className='bg'>
      <h3 align="center" className='h3'><u>Company Registration</u></h3>
      {
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div>
          <label>Since</label>
          <input type="date" id="since" value={formData.since} onChange={handleChange} required />
        </div>
        <div>
          <label>Email</label>
